class Colab < ApplicationRecord
  ...
  def self.buscaColabs (data_inicio, data_final)
     ...
     Colab.where(created_at: (data_inicio)..data_final).find_by_sql('select count(colabs.id), colaboradors.nomecompleto from colabs join colaboradors on colabs.colaborador_id = colaboradors.id group by colaboradors.nomecompleto')
